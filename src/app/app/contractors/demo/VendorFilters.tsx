...
export default function VendorFilters({
  ...
  // Search state matching deals SearchBar.tsx logic
  ...
  const handleSearch = useCallback(() => {
    ...
    router.push(`/app/contractors?${params.toString()}`, { scroll: false });
    router.refresh();
  ...
  const handleRemoveFilters = (keys: string[]) => {
    ...
    router.push(`/app/contractors?${params.toString()}`);
    router.refresh();
  };
  ...
  return (
    <>
      {/* Search Bar and Filter Toggle Layout - matching deals page exactly */}
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 mb-12">
        {/* Search Bar */}
        <div id="vendor-search-bar" className="flex-1 min-w-0">
          <div className="relative w-full group">
            <div className="relative transition-all duration-300 group-focus-within:scale-[1.01]">
              ...
              <input
                ...
                placeholder="Search vendors by name, trade, or service..."
        ...
        {/* Filter Button - matching FilterToggle.tsx styling */}
        <button
          id="vendor-filters-section"
        ...
        >
          ...
          <span>{isOpen ? "Close Filters" : "Filter Vendors"}</span>
          {activeFilterCount > 0 && (
      ...
      {/* Sidebar + Main Content Layout - matching deals page exactly */}
            ...
            <div className="flex h-full flex-col overflow-y-auto p-8 lg:h-auto lg:overflow-visible">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  <h3 className="text-xs font-semibold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">Work Types</h3>
                ...
                {/* Markets */}
                <div className="space-y-4">
                  <h3 className="text-xs font-semibold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">Markets</h3>
                  <div className="space-y-2 max-h-64 overflow-y-auto">
                ...
                <div className="space-y-4">
                  ...
                  <label className="flex cursor-pointer items-center justify-between rounded-lg border border-zinc-200 bg-zinc-50 px-3 py-2.5 text-sm text-zinc-800 transition hover:border-zinc-300 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-200 dark:hover:border-zinc-700">
                    <div>
                      ...
                      <p className="text-xs text-zinc-500 dark:text-zinc-400">Show only vetted vendors</p>
                    </div>
                    <input
                      type="checkbox"
        ...
        <div className="flex-1 w-full">
          ...
          {activeVendors.length === 0 ? (
            <div className="rounded-3xl border border-dashed border-zinc-300 bg-white/40 p-16 text-center shadow-sm dark:border-zinc-700 dark:bg-zinc-900/40">
              ...
              <h3 className="text-lg font-bold text-zinc-900 dark:text-zinc-100 mb-1">No matches found</h3>
              <p className="text-zinc-500 dark:text-zinc-400">
                Try adjusting your filters or clearing them to see more vendors.
              </p>
            </div>
          ) : (
            <div className={`grid gap-6 sm:grid-cols-2 ${isOpen ? "xl:grid-cols-2" : "lg:grid-cols-2 xl:grid-cols-3"}`}>
              {activeVendors.map((vendor) => (
                <VendorCard key={vendor.id} vendor={vendor} id={`vendor-card-${vendor.id}`} />
              ))}
            </div>
          )}
        </div>
