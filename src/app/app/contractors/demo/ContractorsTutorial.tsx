...
export default function ContractorsTutorial() {
    ...
    const steps: TutorialStep[] = [
        {
            title: "Welcome to Vendor Directory",
            content: "Find trusted contractors and service providers for your real estate projects. These pros understand investor timelines and budgets!",
            position: "center"
        },
        {
            targetId: "vendor-search-bar",
            title: "Search by Trade or Name",
            content: "Looking for a plumber, roofer, or general contractor? Type a keyword here to filter the list instantly.",
            position: "bottom"
        },
        {
            targetId: "vendor-filters-section",
            title: "Filter by Market & Trade",
            content: "Narrow down your search by selecting specific markets (like DFW or Austin) and trades to find exactly who you need.",
            position: "bottom"
        },
        {
            targetId: "vendor-card-8d3c63d8-57e3-4428-ba2e-067755c3c0a1", // Lonestar ID
            title: "Verified Vendor Cards",
            content: "Each card shows the vendor's trades, service areas, and verification status. Let's check out Lonestar General Builders to see their full profile.",
            position: "top"
        },
        // Placeholder steps for the detail page (MarketplaceTutorial renders only 0-3)
    ...
    useEffect(() => {
        ...
        if (currentStoredStep !== null) {
            ...
            setStep(0);
            setIsVisible(true);
            localStorage.setItem("active-contractor-tutorial-step", "0");
        }
    }, []);
    ...
    const handleTransitionToDetail = () => {
        setIsVisible(false);
        localStorage.setItem("active-contractor-tutorial-step", "4");
        // Navigate to the Lonestar listing
        const lonestarCard = document.getElementById("vendor-card-8d3c63d8-57e3-4428-ba2e-067755c3c0a1"); // Use the ID we'll add to the card container + Link
        if (lonestarCard) {
            // Find the link inside
            const link = lonestarCard.querySelector('a');
            if (link) link.click();
        } else {
            // Fallback
            router.push("/app/profile/8d3c63d8-57e3-4428-ba2e-067755c3c0a1");
        }
    };
    ...
    const handleSkip = () => {
        ...
        // Redirect to live contractors page
    ...
    return (
        <div className="fixed inset-0 z-[100] pointer-events-none overflow-hidden">
            ...
            <svg className="absolute inset-0 w-full h-full pointer-events-auto">
                <defs>
                    <mask id="contractor-spotlight-mask">
                ...
                <rect
                    ...
                    mask="url(#contractor-spotlight-mask)"
                    onClick={handleSkip}
            ...
            {spotlightRect && (
                <motion.div
                    ...
                    className="absolute top-0 left-0 w-12 h-12 -ml-6 -mt-6 rounded-full bg-emerald-500/30 border border-emerald-400/50 backdrop-blur-md z-[110] pointer-events-none flex items-center justify-center"
                >
