...
import { createPortfolioItem, createReview, requestNetwork, respondNetworkRequest, removeNetworkConnection } from "./actions";
...
// --- Sub-Components (Moved outside to avoid re-creation) ---

const formatRoleLabel = (role: string) => role.replace("_", " ");

const PhotoPost = ({ item, isOwner, profileId }: { item: any, isOwner: boolean, profileId: string }) => {
    ...
    const handleDelete = async () => {
        ...
        await deletePortfolioItem(profileId, item.id);
    };

    return (
        <div className="overflow-hidden bg-zinc-50 dark:bg-black rounded-xl border border-zinc-100 dark:border-zinc-800 relative group text-left">
            {isOwner && (
            ...
            <div className="rounded-2xl border border-yellow-500/30 bg-yellow-500/5 p-6 shadow-sm">
                ...
                <div className="space-y-6 text-sm">
                    ...
                    <div className="space-y-2">
                        <h3 className="font-bold text-zinc-900 dark:text-zinc-100 uppercase text-xs tracking-wider text-left">Contact Information (Admin Only)</h3>
                        <div className="grid grid-cols-1 gap-y-2 p-3 rounded-lg bg-black/20 text-zinc-300">
                            <div className="flex justify-between border-b border-white/10 pb-1">
                                <span className="text-zinc-500">Email:</span>
                                <span className="text-right">{profileData.email || 'N/A'}</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-zinc-500">Phone:</span>
                                <span className="text-right">{profileData.phone || 'N/A'}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    {onboardingData.investor_data && Object.keys(onboardingData.investor_data).length > 0 && (
                        <div className="space-y-2">
                            <h3 className="font-bold text-zinc-900 dark:text-zinc-100 uppercase text-xs tracking-wider text-left">Investor Profile</h3>
                            <div className="grid grid-cols-1 gap-y-2 p-3 rounded-lg bg-black/20 text-zinc-300">
                                <div className="flex justify-between border-b border-white/10 pb-1">
                                ...
                                <div>
                                    <span className="block text-zinc-500 mb-1 text-left">Deal Types:</span>
                    ...
                    {onboardingData.wholesaler_data && Object.keys(onboardingData.wholesaler_data).length > 0 && (
                        <div className="space-y-2">
                            <h3 className="font-bold text-zinc-900 dark:text-zinc-100 uppercase text-xs tracking-wider text-left">Wholesaler Stats</h3>
                            <div className="grid grid-cols-1 gap-y-2 p-3 rounded-lg bg-black/20 text-zinc-300">
                                <div className="flex justify-between border-b border-white/10 pb-1">
                    ...
                    {(onboardingData.vendor_data?.companyName || onboardingData.service_data?.companyName) && (
                        <div className="space-y-2">
                            <h3 className="font-bold text-zinc-900 dark:text-zinc-100 uppercase text-xs tracking-wider text-left">Business Details</h3>
                            <div className="grid grid-cols-1 gap-y-2 p-3 rounded-lg bg-black/20 text-zinc-300">
                    ...
                    <div className="space-y-4 text-sm">
                        <div className="flex items-start gap-3 text-zinc-600 dark:text-zinc-400">
                            ...
                            <div className="flex-1">
                                <span className="block mb-2 text-zinc-500">Roles:</span>
                                {onboardingData && onboardingData.roles && onboardingData.roles.length > 0 ? (
                                    <div className="flex flex-wrap gap-2">
                                        {onboardingData.roles.map((role: string) => {
                                            const roleColors: Record<string, string> = {
                                                investor: "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 border-blue-200 dark:border-blue-800",
                                                wholesaler: "bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300 border-purple-200 dark:border-purple-800",
                                                service: "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300 border-green-200 dark:border-green-800",
                                                vendor: "bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-300 border-orange-200 dark:border-orange-800",
                                                admin: "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300 border-red-200 dark:border-red-800",
                                            };
                                            const colorClass = roleColors[role] || "bg-zinc-100 text-zinc-700 dark:bg-zinc-800 dark:text-zinc-300 border-zinc-200 dark:border-zinc-700";
                                            return (
                                                <span
                                                    key={role}
                                                    className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold border capitalize ${colorClass}`}
                                                >
                                                    {role}
                                                </span>
                                            );
                                    ...
                                    <span className="font-semibold text-zinc-900 dark:text-zinc-200 capitalize">{formatRoleLabel(profileData.role)}</span>
                                )}
                            </div>
                        </div>
                    ...
                    <div className="space-y-8">
                        {portfolioItems.length > 0 ? (
                            portfolioItems.map(item => ({ ...item, images: item.images && item.images.length > 0 ? item.images : [item.image_url] })).map(item => (
                                <PhotoPost key={item.id} item={item} isOwner={isOwner} profileId={profileData.id} />
                            ))
                        ) : (
...
const TabNetwork = ({
    ...
    <div className="rounded-2xl border border-zinc-200 bg-white p-8 shadow-sm dark:border-zinc-800 dark:bg-zinc-950 text-left">
        ...
        {networkConnections.length > 0 ? (
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {networkConnections.map((conn) => {
                    const otherUser = conn.requester_id === profileId ? (Array.isArray(conn.requestee) ? conn.requestee[0] : conn.requestee) : (Array.isArray(conn.requester) ? conn.requester[0] : conn.requester);
            ...
            <div className="relative mb-6 bg-white dark:bg-zinc-900 shadow-sm sm:rounded-b-3xl">
                ...
                <div className="px-4 pb-4 sm:px-8">
                        ...
                        <div className="flex flex-col items-center gap-4 sm:flex-row sm:items-end">
                            ...
                            <div className="mt-2 text-center sm:mb-2 sm:mt-0 sm:text-left">
                                ...
                                {profileData.business_name && (
                                    <p className="text-zinc-500 font-medium text-sm mt-0.5">{profileData.business_name}</p>
                                )}
                                {/* Role Badges */}
                                {onboardingData && onboardingData.roles && onboardingData.roles.length > 0 ? (
                                    <div className="flex flex-wrap items-center justify-center gap-2 mt-2 mb-1 sm:justify-start">
                                        {onboardingData.roles.map((role: string) => {
                                            const roleColors: Record<string, string> = {
                                                investor: "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 border-blue-200 dark:border-blue-800",
                                                wholesaler: "bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300 border-purple-200 dark:border-purple-800",
                                            ...
                                            return (
                                                <span
                                                    key={role}
                                                    className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold border capitalize ${colorClass}`}
                                                >
                                                    {role}
                                                </span>
                                            );
                                ...
                                {/* Network Count */}
                                <div className="flex flex-wrap items-center justify-center gap-2 text-sm font-medium text-zinc-600 dark:text-zinc-400 sm:justify-start mt-1">
                                ...
                                <>
                                    {networkRequest?.status === "accepted" ? (
                                        <div className="group relative flex-1 sm:flex-none">
                                            <button
                                                className="w-full flex items-center justify-center gap-2 rounded-lg bg-green-500 px-5 py-2 text-sm font-bold text-white shadow-lg shadow-green-500/20 transition-all sm:min-w-[140px]"
                                            >
                                                <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            ...
                                            {/* Disconnect Option on Hover */}
                                            <div className="absolute right-0 top-full z-10 mt-2 hidden w-48 rounded-xl border border-zinc-200 bg-white p-1 shadow-xl group-hover:block dark:border-zinc-800 dark:bg-zinc-900 text-left">
                                                <button
                                                    onClick={async () => {
                                                        if (confirm("Are you sure you want to remove this connection?")) {
                                                            await removeNetworkConnection(profileData.id);
                                                        }
                                                    }}
                                                    className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs font-bold text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20"
                                                >
                    ...
                    {isOwner && pendingRequests.length > 0 && (
                        <div className="mt-4 mb-2 rounded-xl bg-purple-50 p-4 dark:bg-purple-900/20 text-left">
